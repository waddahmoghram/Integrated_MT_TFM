function [CurrentFramePlot] = plotTractionHeatmapsVectorsParforROI(MD_EPI,forceField,CurrentFrame,  QuiverScaleToMax, QuiverColor, colormapLUT_parula, ...
        ~, FramesNumEPI, ScaleLength_EPI, ScaleMicronPerPixel_EPI, TimeStampsRT_Abs_EPI, FluxStatusString,  reg_grid, ...ROIMagnificationFactor
        InterpolationMethod, bandSize, colorbarLimits, colorbarFontSize, reg_corner_averaged, tractionInfoTxt,  useGPU, MaxTractionNetPa, ROIsideSizeMicron, ROIMagnificationFactor)
...
%     if useGPU, grid_mat = gather(grid_mat);displVecGridNorm = gather(displVecGridNorm); end        
    ...
    X = reshape(grid_matX,1,[]) * ScaleMicronPerPixel_EPI;
    Y = reshape(grid_matY,1,[]) * ScaleMicronPerPixel_EPI;
    U = reshape(gather(dMapX),1,[]) * ScaleMicronPerPixel_EPI;
    V = reshape(gather(dMapY),1,[]) * ScaleMicronPerPixel_EPI;
    ...
    figHandleROI = figure('color','w', 'Units','pixels', 'visible', 'off');
