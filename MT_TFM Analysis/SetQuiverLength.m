...
%--------------------------------------------------
% Modified by Waddah Moghram on 2020-02-08 to keep scales are they are, but magnify the heads
...
%// Direction tail
% Tail_dir = diff(Tail_ori,1,2)./permute(Tail_length(:,:,ones(size(T.VertexData,1),1)),[3 2 1]);    % commented out 2020-02-08
...
TailVertex = Tail_ori;
% line below commented out by WIM on 2020-02-08
% TailVertex(:,2,:) = TailVertex(:,1,:) + permute(mags(:,:,ones(size(T.VertexData,1),1)),[3 2 1]) .*  Tail_dir; 
