function [MaxEnergyDetails, outputPlotFileNameFig] = ExtractBeadMaxEnergyGrid(MD, energyDensityField, AnalysisFolderAsWell)
%% 
%{   
...
%             LastFrameSearch = FramesDoneNumbers(LastFrameIndex);
    ...
    reverseString = '';
    [reg_grid, ~, ~, GridSpacing] = createRegGridFromDisplField(energyDensityField(FramesDoneNumbers(1)).pos, gridMagnification, EdgeErode);  
    ...
    for CurrentFrame = FramesDoneNumbersSearch
        ...
        EnergyNormGrid = energyDensityField(CurrentFrame).vec;
        [grid_mat, EnergyNormGrid,~,~] = interp_vec2grid(energyDensityField(CurrentFrame).pos(:,1:2), EnergyNormGrid ,[], reg_grid);
...
%         [XI, YI] = meshgrid(Xmin:Xmax,Ymin:Ymax);
        ...
        reg_gridFull(:,:,1)  = XI; reg_gridFull(:,:,2)  = YI;
        [grid_mat_full, EnergyHeatMapFullNorm,~,~] = interp_vec2grid(energyDensityField(CurrentFrame).pos(:,1:2), energyDensityField(CurrentFrame).vec ,[], reg_gridFull, 'griddata');    

   
        [tmpMaxEnergyFieldNetInFrame, tmpMaxEnergyFieldInFrameIndex] =  max(EnergyHeatMapFullNorm(:));          % maximum item in a column
        ...
        if tmpMaxEnergyFieldNetInFrame > MaxEnergyPaNet
            ...
            MaxEnergyPaXYnet = EnergyHeatMapFullNorm(MaxEnergyIndex);
            [MaxPosXYnet(1),MaxPosXYnet(2)] = ind2sub(size(EnergyHeatMapFullNorm), tmpMaxEnergyFieldInFrameIndex);
        end 
    end
    fprintf('Maximum Energy Density = %g fJ/micron^2 at ', MaxEnergyPaNet);
    fprintf('[x,y] = [%g, %g] pixels in Frame #%d, Point Index #%d \n', MaxPosXYnet, MaxEnergyFrameNumber, MaxEnergyIndex)
    fprintf('Maximum Energy [sigma_x, sigma_y] = %g fJ/micron^2. \n', MaxEnergyPaXYnet)
...
%         [XI, YI] = meshgrid(Xmin:Xmax,Ymin:Ymax);
        ...
        TxRedBeadMaxNetPositionPixels(CurrentFrame, :) =  MaxPosXYnet;
        TxRedBeadMaxNetEnergyPa(CurrentFrame) =  EnergyHeatMapFullNorm(MaxEnergyIndex);
    end
    ...
    title({'Maximum EPI Bead Energy (Interpolated)',sprintf('Max at (X,Y) = (%d,%d) pix in Frame %d/%d = %0.3f sec',...
        MaxPosXYnet, MaxEnergyFrameNumber,LastFrame, TimeStamps(MaxEnergyFrameNumber)), ...
        sprintf('Max Energy Density = %g fJ/\\mum^{2}', MaxEnergyPaXYnet)})
    ...
    %%
    outputPlotFileNameFig = fullfile(FramesOutputPath, sprintf('Energy_TxRed_max_interp_%s.fig', TimeStampChoice));
%     outputPlotFileNameTIF = fullfile(FramesOutputPath,  sprintf('Energy_TxRed_max_interp_%s.tif', TimeStampChoice));
    outputPlotFileNamePNG = fullfile(FramesOutputPath,  sprintf('Energy_TxRed_max_interp_%s.png', TimeStampChoice));
    outputPlotFileNameMAT = fullfile(FramesOutputPath,  sprintf('Energy_TxRed_max_interp_%s.mat', TimeStampChoice));
...
%     imwrite(FrameImage.cdata, outputPlotFileNameTIF);
    ...
    if AnalysisFolderAsWell
        outputPlotFileNameFigAnalysis = fullfile(AnalysisOutputPath, sprintf('07 Energy_TxRed_max_interp_%s.fig', TimeStampChoice));
    %     outputPlotFileNameTIFAnalysis = fullfile(AnalysisOutputPath, sprintf('07 Energy_TxRed_max_interp_%s.tif', TimeStampChoice));
        outputPlotFileNamePNGAnalysis = fullfile(AnalysisOutputPath, sprintf('07 Energy_TxRed_max_interp_%s.png', TimeStampChoice));
        outputPlotFileNameMATAnalysis = fullfile(AnalysisOutputPath, sprintf('07 Energy_TxRed_max_interp_%s.mat', TimeStampChoice));
