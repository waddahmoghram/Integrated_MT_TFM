...
%% Video 1: displ corrected + LPEF + DC (quivers)
    disp('----------------------------------------------------------------------------------------------------------------')
    ...
    disp('1.0 Calculating displacements in microns IN PROGRESS')
    diary off
    ...
    diary on
    disp('1.0 Calculating displacements in microns COMPLETE')
...
%% Video 3: traction stress from grid (from displ outliercorrection, LPEF, DC)
    disp('------------------------------------------------------------------------------')
...
%% Video 4: displ corrected + LPEF onl (tracked beads)
    disp('----------------------------------------------------------------------------------------------------------------')
    ...
    [VideoPathName1, VideoFileNameSuffix, ~] = fileparts(displacementFileFullNameRaw);   
    VideoFileName = strcat(VideoFileNameSuffix, '_tracked');
    VideoFullFileName1 = fullfile(VideoPathName1, VideoFileName);
    VideoWriterObj1 = VideoWriter(VideoFullFileName1, VideoType);
    if any(strcmpi(VideoType, {'Motion JPEG AVI', 'MPEG-4'})), VideoWriterObj1.Quality = 100; end
    VideoFullFileName1 = fullfile(VideoPathName1, VideoWriterObj1.Filename);
    VideoWriterObj1.FrameRate = FrameRateRT_EPI; 
    displFieldPath = VideoPathName1;


% Video 1:==================================================================
    fprintf("-------- Video 1.  tracked beads: %s ------------------\n", VideoWriterObj1.Filename)
...
%     YI = gather(YI);            
    ...
    MaxDisplFieldIndexXY(1,2) = YI(MaxDisplFieldIndex);
    [MaxDispl_PosXY_Pixels(1,1), MaxDispl_PosXY_Pixels(1,2)] =  ind2sub(size(d_norm), MaxDisplFieldIndex);
    MaxDisplNetPixels = [displVecGridXY(MaxDispl_PosXY_Pixels(1),MaxDispl_PosXY_Pixels(2),1), displVecGridXY(MaxDispl_PosXY_Pixels(1),MaxDispl_PosXY_Pixels(2),2), MaxDisplNetMicrons(3)];
    MaxDisplNetMicrons = MaxDisplNetPixels * ScaleMicronPerPixel_EPI;    
    fprintf('[x,y] = [%0.3f, %0.3f] pixels in Frame #%d, Point Index #%d \n', MaxDisplFieldIndexXY, MaxDisplFrameNumber, MaxDisplFieldIndex)
    fprintf('Maximum displacement [disp_x, disp_y] =  [%0.3f, %0.3f] pixels  \t==> Net displacement [disp_net] = [%0.3f] pixels. \n', MaxDisplNetPixels)
    fprintf('Maximum displacement [disp_x, disp_y] =  [%0.3f, %0.3f] %s      \t==> Net displacement [disp_net] = [%0.3f] %s. \n', ...
...
%% Video /????: 
    ...
    displacementFileFullNameRaw = MD_EPI.findProcessTag(displFieldProcess).outFilePaths_{1};  
    load(displacementFileFullNameRaw, 'displField');                % at this point. displField.mat is the default file
    fprintf('Corrected Displacement Field (displField) File is successfully loaded!:\n\t%s\n', displacementFileFullNameRaw);
    ...
    fprintf("-------- Video 3.  tracked beads (spatial outlier corrected): %s ------------------\n", VideoWriterObj1.Filename)



    %% Video 4.  tracked beads (spatial outlier corrected + Low-pass equirriples ) heatmap
    disp('----------------------------------------------------------------------------------------------------------------')
    displFieldProcess =  MD_EPI.findProcessTag('DisplacementFieldCorrectionProcess').tag_;
    displacementFileFullNameRaw = MD_EPI.findProcessTag(displFieldProcess).outFilePaths_{1};  
...
%% Video 5.  tracked beads (spatial outlier corrected + Low-pass equirriples ) heatmap
    disp('----------------------------------------------------------------------------------------------------------------')
    displFieldProcess =  MD_EPI.findProcessTag('DisplacementFieldCorrectionProcess').tag_;
    displacementFileFullNameRaw = MD_EPI.findProcessTag(displFieldProcess).outFilePaths_{1};  
    [DisplPathName, displFileNamePrefix, displFieldFileNameExt] = fileparts(displacementFileFullNameRaw);   
...
%     VideoWriterObj = VideoWriter(VideoFullFileName, VideoType);
%     if any(strcmpi(VideoType, {'Motion JPEG AVI', 'MPEG-4'})), VideoWriterObj.Quality = 100; end
...
%     fprintf("--------Video 3.  displacement vectors: %s ------------------\n", VideoWriterObj.Filename)
    ...
    disp('5.1 Finding the maximum displacement vector in ON/Transient frames IN PROGRESS')
    FramesNumEPI = numel(displField);
    FramesDoneNumbers = 1:FramesNumEPI;
    dmaxTMP = nan(FramesNumEPI, 1);
    dmaxTMPindex = nan(FramesNumEPI, 1);    
    ...
    fprintf('Maximum displacement [disp_x, disp_y] =  [%0.3f, %0.3f] %s      \t==> Net displacement [disp_net] = [%0.3f] %s. \n', ...
        MaxDisplNetMicrons(1:2), displUnit, MaxDisplNetMicrons(3) , displUnit)
    disp('5.1 Finding the bead with the maximum displacement in ON/Transient frames COMPLETE')
...
%% Video 6: 
    ...
    if any(strcmpi(VideoType, {'Motion JPEG AVI', 'MPEG-4'})), VideoWriterObj1.Quality = 100; end
    VideoFullFileName1 = fullfile(VideoPathName1, VideoWriterObj1.Filename);
    VideoWriterObj1.FrameRate = FrameRateRT_DIC; 
    forceFieldPath = VideoPathName1;
