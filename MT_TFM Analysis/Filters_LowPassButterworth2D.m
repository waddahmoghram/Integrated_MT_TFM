function [pos_grid_trimmed, disp_grid_trimmed, stress_grid_trimmed, i_max_windowed, j_max_windowed, qMax] = Filters_LowPassButterworth2D(pos_grid, disp_grid_NotFiltered, gridSpacing, z, h, YoungModulusPa, PoissonRatio, ...
    fracPad, BW_order, reg_corner, i_max, j_max)
%{
    ...
    FTTC Solution is included in this code.
%}
 %% ========  Check if there is a GPU. take advantage of it if is there. ============================================================  
    ...
    if ~exist('BW_order', 'var'), BW_order = []; end
    if nargin < 8 || isempty(BW_order)
        BW_order = 1;
    end
...
%     pos_grid = padarray(pos_grid, [padwidth, padheight]);
...
%     Q = calcQ(z,h,YoungModulusPa, PoissonRatio,nr2,gridSpacing,2); 
%     tmpStressField = disp2stress(tmpDisplField,Q);
    ...
    if mod(nr,2)==0     
        [~,~, stress_vector, stress_vector_norm] = reg_fourier_TFM([], disp_grid_Windowed(:,:,1:2), YoungModulusPa,...
            PoissonRatio, [], gridSpacing, i_max_windowed, j_max_windowed, reg_corner);
            stress_vector(:,3) = stress_vector_norm;    
    else
        [~,~, stress_vector, stress_vector_norm] = reg_fourier_TFM_odd([], disp_grid_Windowed(:,:,1:2), YoungModulusPa,...
            PoissonRatio, [], gridSpacing, i_max_windowed, j_max_windowed, reg_corner);
            stress_vector(:,3) = stress_vector_norm;                               
    end
...
%     if useGPU
%         disp_grid_Windowed = gather(disp_grid_Windowed);
...
%     sed = 1/2*d(i).stress_x.*d(i).ux + 1/2*d(i).stress_y.*d(i).uy;               % strain energy. Mertz et al. PRL 108, 198101 (2012).
    ...
    if useGPU
        pos_grid_trimmed = gather(pos_grid);
        stress_grid_trimmed = gather(stress_grid_trimmed);
