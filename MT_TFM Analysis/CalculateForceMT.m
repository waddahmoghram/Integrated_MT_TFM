...
%     xlim([TimeStampsSeconds(1), TimeStampsSeconds(end)]);
    ...
    title({'Separation Distance vs. Time', titleStr});
...
%     ForceInfo = sprintf('MT Force on M450 bead at inclination angle of %.0f', InclinationAnglesDegrees);    
%     ForceInfo = [ForceInfo, char(176)];        % Add a degree at the end
    %     title({ForceInfo, titleStr2})   
    title({'Magnetic Tweezer Pulling Force vs. Time', titleStr});   
...
%     xlim([TimeStampsSeconds(1), TimeStampsSeconds(end)]);
    ...
    %     title({ForceInfo, titleStr2})   
    title({'Inclination angle between the bead and the needle tip', titleStr});
...
%     xlim([TimeStampsSeconds(1), TimeStampsSeconds(end)]);
    ...
    %     title({ForceInfo, titleStr2})   
    title({'Bead Displaement/Force XY-vector angle with respect to x-axis', titleStr});
...
%     xlim([TimeStampsSeconds(1), TimeStampsSeconds(end)]);
    ...
    %     title({ForceInfo, titleStr2})   
    title({'Magnetic Tweezer Force Error. ',  strcat(sprintf('Error_{Calibration} = %0.3g nN. Error_{x,y,z} = %0.3g ', Error_Calibration, Error_x), char(181), 'm'), titleStr});  
