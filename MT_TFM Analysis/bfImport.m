...
% Modified by Waddah Moghram on 2020-03-12 to handle ome.units.UNITS.NM error
...
ip.addParameter('askUser', false, @isscalar);
ip.addParameter('class',@MovieData,@(c) ischar(c) || isa(c,'function_handle'));
...
if ~isempty(pixelSizeX)
    try
        ...
        pixelSize = double(pixelSizeX.getValue) * 1000;                            % Fixed by WIM on 2020-02-26 to nm from microns.
    end
end
...
if ~isempty(pixelSizeY)
    if ~isempty(pixelSize)
        try
            ...
            pixelSizeY =  double(pixelSizeY.getValue) * 1000;                      % Fixed by WIM on 2020-02-26 to nm from microns
        end
...
if ~isempty(pixelSizeZ)
    try
        ...
        pixelSizeZ =  double(pixelSizeZ.getValue) * 1000;                      % Fixed by WIM on 2020-02-26 to nm from microns.
    end
