...
function [BeadNetDisplacementMicronEPImax_EachFrame, MD, displField, FirstFrame, LastFrame, movieFilePath, displacementFilePath, AnalysisPath, NetDisplacementPath, BeadNetDisplacementMicronEPImax_Absolute] = PlotBeadEpiMaxDisplacement_Velocity(MD, displField, ...
...
%         end    
        % -----------------------------------------------------------------------------------------------
        [grid_mat, interpGridVector_Displ,~,~] = interp_vec2grid(displFieldMicron(CurrentFrame).pos(:,1:2), displFieldMicron(CurrentFrame).vec ,[], reg_grid);
        [~, interpGridVector_Velocity,~,~] = interp_vec2grid(velocityFieldMicronPerSec(CurrentFrame).pos(:,1:2), velocityFieldMicronPerSec(CurrentFrame).vec ,[], reg_grid);        
        %-----------------------------------------------------------------------------------------------
