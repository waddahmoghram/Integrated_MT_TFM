...
%                 quiver(Field(k).pos(inIdx,1)-xmin,Field(k).pos(inIdx,2)-ymin, Field(k).vec(inIdx,1)./dispScale,Field(k).vec(inIdx,2)./dispScale,0,'Color',[75/255 0/255 130/255],'LineWidth',0.5);
                quiver(Field(k).pos(inIdx,1)-xmin,Field(k).pos(inIdx,2)-ymin, Field(k).vec(inIdx,1)./dispScale,Field(k).vec(inIdx,2)./dispScale,0,'Color',[255/255 255/255 255/255],'LineWidth',0.5);
            end
            ...
            if ~isempty(dataPath)
                ...
                print(h3,strcat(epsPath,'/displFieldMagEPS',num2str(k),'.eps'),'-depsc')
                close(h3)
            end
        end
    end
