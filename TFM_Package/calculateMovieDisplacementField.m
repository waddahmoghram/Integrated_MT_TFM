...
%----------------------------------
    ...
    %----------------------------------
        if ~isempty(SDCProc)
            currImage = double(SDCProc.loadChannelOutput(displacementParameters.ChannelIndex(1),j));
        else
            currImage = double(movieData.channels_(displacementParameters.ChannelIndex(1)).loadImage(j));
        end
    %----------------------------------
        if ~displacementParameters.useGrid
...
%                 scoreCalculation='difference';
                ...
                if j== 1
                    ...
                    if ~exist('prevImage', 'var')
                        if ~isempty(SDCProc)
                            prevImage = double(SDCProc.loadChannelOutput(displacementParameters.ChannelIndex(1), j - 1));
                        else
                            prevImage = double(movieData.channels_(displacementParameters.ChannelIndex(1)).loadImage(j - 1));
                        end
                    end
                end
                %----------------------------------
...
%                 currentBeads = currentBeads + v2;                             % Modified by Waddah Moghram on 2019-10-27
    ...
    %         pivData.V=zeros(size(pivData.Y));
        ...
        %------ Modified by Waddah Moghram on 2019-04-07. Added (validV)
            if ~displacementParameters.trackSuccessively
                displField(j).pos=[pivData.X(validV), pivData.Y(validV)];                                          % Added validV. WIM on 4/7/2019
