function  [rho, eta, reg_corner, alphas, figHandleRegParam] = calculateLcurveFTTC(grid_mat, u, E, s, cluster_size, i_max, j_max, L, LcurveFactor, ShowPlot)
%{
...
%         figHandleRegParam = [];
    ...
    if useGPU
        try
            LcurveFactor = gpuArray(LcurveFactor);
            L = gpuArray(L);
            grid_mat = gpuArray(grid_mat);
            u = gpuArray(u);
        catch
           % continue. Do not use GPU
           useGPU = false;
        end            
    end
...
%     alphas = 10.^(log10(L)-5:1.25/LcurveFactor:log10(L)+2.5);
    L = 1;LcurveFactor = 10;                                    % Added on 2020-05-10 by WIM
    alphas = 10.^(log10(L)-6:2.5/LcurveFactor:log10(L)+6);      % updated on 2020-05-10 by WIM
...
%         disp(['(calculateLcurveFTTC.m): Testing L = ' num2str(alphas(i)) '... '])
        curL=alphas(i);
        V = 2*(1+s)/E;
...
%     [reg_corner,~,~]=regParamSelecetionLcurve(rho,eta,alphas,L);
    ...
    [reg_corner,ireg_corner,~, figHandleRegParam] = regParamSelecetionLcurve(alphas', eta, alphas, L, 'manualSelection', ShowPlot);
    
    fprintf('\tReg_corner L2-Norm   = %7.5g. ', reg_corner);         
    fprintf('\t\tReg_corner L2-Norm = %g seconds. \n', toc(start));
