...
StageCorrectionProcess = movieData.processes_{iProc};                   % Changed from displFieldProce 2019-08-21
%Parse input, store in parameter structure
StageCorrectionProcessParameters = parseProcessParams(StageCorrectionProcess,paramsIn);             % Change p to StageCorrectionProcessParameters. 2019-08-21

%% Backup the original vectors to backup folder
if exist(StageCorrectionProcessParameters.OutputDirectory,'dir')
    disp('Backing up the original data')
    ii = 1;
    backupFolder = [StageCorrectionProcessParameters.OutputDirectory ' Backup ' num2str(ii)];
    ...
    copyfile(StageCorrectionProcessParameters.OutputDirectory, backupFolder,'f')
end
mkClrDir(StageCorrectionProcessParameters.OutputDirectory);
%% --------------- Initialization ---------------%%
if feature('ShowFigureWindows')
    wtBar = waitbar(0,'Initializing...','Name',StageCorrectionProcess.getName());
else
    wtBar=-1;
end
...
for j = StageCorrectionProcessParameters.ChannelIndex
    inFilePaths{1,j} = imDirs{j};
end
StageCorrectionProcess.setInFilePaths(inFilePaths);
    
% Set up the output directories
outFilePaths = cell(3,numel(movieData.channels_));
mkClrDir(StageCorrectionProcessParameters.OutputDirectory);
for i = StageCorrectionProcessParameters.ChannelIndex;    
    %Create string for current directory
    outFilePaths{1,i} = [StageCorrectionProcessParameters.OutputDirectory filesep 'channel_' num2str(i)];
...
outFilePaths{2,StageCorrectionProcessParameters.ChannelIndex(1)} = [StageCorrectionProcessParameters.OutputDirectory filesep refName refExt];
outFilePaths{3,StageCorrectionProcessParameters.ChannelIndex(1)} = [StageCorrectionProcessParameters.OutputDirectory filesep 'transformationParameters.mat'];
StageCorrectionProcess.setOutFilePaths(outFilePaths);

%---------- Added by Waddah Moghram on 2019-08-21, on Friday 2019-12-13  (FRIDAY THE 13TH!!!)
outFilePaths{4,StageCorrectionProcessParameters.ChannelIndex(1)} = [StageCorrectionProcessParameters.OutputDirectory filesep 'StageCorrectionParameters.mat'];
outFilePaths{5,StageCorrectionProcessParameters.ChannelIndex(1)} = [StageCorrectionProcessParameters.OutputDirectory filesep 'movieDataBefore.mat'];
outFilePaths{5,StageCorrectionProcessParameters.ChannelIndex(1)} = [StageCorrectionProcessParameters.OutputDirectory filesep 'movieDataAfter.mat'];
%---------------------------
...
refFrame = double(imread(StageCorrectionProcessParameters.referenceFramePath));
croppedRefFrame = imcrop(refFrame,StageCorrectionProcessParameters.cropROI);

% Detect beads in reference frame
StageCorrectionProcessParameters.doSubPixReg = true;
beadsChannel = movieData.channels_(StageCorrectionProcessParameters.ChannelIndex(1));

%----------- added by WIM on 2019-12-13
movieDataBefore = movieData;
save(outFilePaths{4,StageCorrectionProcessParameters.ChannelIndex(1)},'StageCorrectionProcess', 'StageCorrectionProcessParameters', 'movieDataBefore', '-v7.3');
MD = movieData;
save(outFilePaths{5,StageCorrectionProcessParameters.ChannelIndex(1)}, 'MD', '-v7.3')
%------------------
...
if StageCorrectionProcessParameters.doPreReg % Perform pixel-wise registration by auto-correlation
...
if StageCorrectionProcessParameters.doSubPixReg
    disp('Determining PSF sigma from reference frame...')
    
    % Adaptation of psfSigma from bead channel image data. Modified by Waddah Moghram on 2019-10-29 
    [~, psfSigma] = getGaussianPSFsigmaFromData(refFrame,'Display',true);
    
    if isnan(psfSigma) || psfSigma>movieData.channels_(1).psfSigma_*3  
        if strcmp(movieData.getChannel(StageCorrectionProcessParameters.ChannelIndex(1)).imageType_,'Widefield') || movieData.pixelSize_>130
            psfSigma = movieData.channels_(1).psfSigma_*2; %*2 scale up for widefield
        elseif strcmp(movieData.getChannel(StageCorrectionProcessParameters.ChannelIndex(1)).imageType_,'Confocal')
            psfSigma = movieData.channels_(1).psfSigma_*0.79; %*4/7 scale down for  Confocal finer detection SH012913
        elseif strcmp(movieData.getChannel(StageCorrectionProcessParameters.ChannelIndex(1)).imageType_,'TIRF')
            psfSigma = movieData.channels_(1).psfSigma_*3/7; %*3/7 scale down for TIRF finer detection SH012913
        else
            error('(ERROR in correctMovieStageDrift.m): image type should be chosen among Widefield, confocal and TIRF!');
        end
    ...
    assert(~isempty(psfSigma), ['Channel ' num2str(StageCorrectionProcessParameters.ChannelIndex(1)) ' have no '...
...
%     pstruct = pointSourceDetection(croppedRefFrame, psfSigma, 'alpha', StageCorrectionProcessParameters.alpha);
    pstruct = pointSourceDetection(croppedRefFrame, psfSigma, 'alpha', StageCorrectionProcessParameters.alpha,'FitMixtures', false);             % Changed by Waddah Moghram on 2019-10-28
    ...
    erosionDist=ceil((StageCorrectionProcessParameters.minCorLength+1+floor(StageCorrectionProcessParameters.maxFlowSpeed))/2);
...
if StageCorrectionProcessParameters.doSubPixReg
    ...
    for j= 1:nFrames
        ...
        corrStack =cat(3,imcrop(refFrame,StageCorrectionProcessParameters.cropROI),imcrop(stack(:,:,j),StageCorrectionProcessParameters.cropROI));

        delta = trackStackFlow(corrStack,beads,...
            StageCorrectionProcessParameters.minCorLength,StageCorrectionProcessParameters.minCorLength,'maxSpd',StageCorrectionProcessParameters.maxFlowSpeed,'mode','accurate');
...
nChan = length(StageCorrectionProcessParameters.ChannelIndex);
nTot = nChan*nFrames;
for i = 1:numel(StageCorrectionProcessParameters.ChannelIndex)
    iChan = StageCorrectionProcessParameters.ChannelIndex(i);
    % Log display
...
imwrite(uint16(refFrame), outFilePaths{2,StageCorrectionProcessParameters.ChannelIndex(1)});
if StageCorrectionProcessParameters.doSubPixReg
    save(outFilePaths{3,StageCorrectionProcessParameters.ChannelIndex(1)},'preT','T','flow');
else
    save(outFilePaths{3,StageCorrectionProcessParameters.ChannelIndex(1)},'preT','T');
end

%----------- Added by Waddah Moghram on 5/27/2019
movieDataAfter = movieData;
save(outFilePaths{4,StageCorrectionProcessParameters.ChannelIndex(1)}, 'movieDataAfter', '-append');
MD = movieData;
save(outFilePaths{6,StageCorrectionProcessParameters.ChannelIndex(1)},MD);
%-------------------------------
