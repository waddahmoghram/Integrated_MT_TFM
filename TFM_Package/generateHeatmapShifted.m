function [tMap, tmax, tmin, cropInfo,tMapX,tMapY,reg_grid1] = generateHeatmapShifted(forceField, displField, band)
    ...
    %{
    Originally generates an image of traction in the place of deformed position defined by displField. 
    **** No longer shifted heat map. To return to original, uncomment line 112 and comment line 114 ***** by Waddah Moghram on 2/17/2019
    input: 
...
%     disp(['(generateHeatmapShifted.m): Estimated force maximum = ' num2str(tmax) ' Pa.'])
    ...
    progressText(0,'(generateHeatmapShifted.m): Traction map creation: ') % Create text
    ...
    for ii=1:numel(forceField)
        ...
        try
            ...
            if ii>1
                disp(['(generateHeatmapShifted.m): Too many NaNs in the field in ' num2str(ii) 'th frame. Assigining values in the ' num2str(ii-1) 'th frame ...'])
                tMap{ii} = (tmat(:,:,1).^2 + tmat(:,:,2).^2).^0.5;
                tMapX{ii} = tmat(:,:,1);
                tMapY{ii} = tmat(:,:,2);
                continue
            else
                disp(['(generateHeatmapShifted.m): Too many NaNs in the field in ' num2str(ii) 'th frame. Assigining zeros ...'])
...
%     [~,iu_mat,~,~] = interp_vec2grid(displField(ii).pos, displField(ii).vec,[],reg_grid1);
        ...
        force_vec = [reshape(if_mat(:,:,1),[],1) reshape(if_mat(:,:,2),[],1)]; 
         ...
         %% This is the part where the shift happens (+disp_vec). 
        % [~,tmat, ~, ~] = interp_vec2grid(pos+disp_vec, force_vec,[], reg_grid); %1:cluster size    
        % Modified by WIM so that it is not shifted. Uncomment the part above so that it becomes shifted.
       [~,tmat, ~, ~] = interp_vec2grid(pos, force_vec,[], reg_grid); %1:cluster size
