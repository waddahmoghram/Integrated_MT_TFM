function varargout = forceFieldCalculationProcessGUI(varargin)
    ...
    %{
     forceFieldCalculationProcessGUI, by itself, creates a new forceFieldCalculationProcessGUI or raises the existing singleton*.
    ...
    v.2019-12-16, Updated by Waddah Moghram, PhD Student in Biomedical Engineering at the University of Iowa.
        1. Added funParams.PadRandomZeros and funParams.HanWindow to get those values from forceFieldCalculationProcessGUI.fig
    v.2019-02-04, Updated by Waddah Moghram, PhD Student in Biomedical Engineering at the University of Iowa.
        1. Enable the selection for Poisson Ratio on forceFieldCalculationProcessGUI.fig, and adjusted code to accept that value.
    %} 

    % Edit the above text to modify the response to help forceFieldCalculationProcessGUI

    % Last Modified by GUIDE v2.5 16-Dec-2019 15:31:13
...
function forceFieldCalculationProcessGUI_OpeningFcn(hObject,eventdata,handles,varargin)
    ...
    %____________________ Added by Waddah Moghram on 2019-12-16
    set(handles.edit_WienerWindowSize, 'String', num2str(funParams.WienerWindowSize))
    set(handles.PadRandomZeros, 'Value', funParams.PadRandomZeros);
    set(handles.WienerFilter, 'Value', funParams.WienerFilter);
    set(handles.HanWindow, 'Value', funParams.HanWindow);
...
function pushbutton_done_Callback(hObject, eventdata, handles)
    ...
    funParams.YoungModulus = str2double(get(handles.edit_YoungModulus,'String'))*1000;                     % Converting from kPa to Pa

    %_______ Added by Waddah Moghram on 2019-02-04 after enabling the Poisson Ratio for my Collagen Type-I Experiments.
    % Open 'forceFieldCalculationProcessGUI.m' using GUIDE and enable "edit_PoissonRatio" in Property Inspector
    if isempty(get(handles.edit_PoissonRatio,'String'))
        errordlg(['Please enter a valid value for ' ...
            get(handles.text_PoissonRatio,'String') '.'],'Setting Error','modal');
        return;
    end
    funParams.PoissonRatio = str2double(get(handles.edit_PoissonRatio,'String'));
    %________________
    ...
    %____________ Added by Waddah Moghram on 2019-12-16    
    funParams.PadRandomZeros = get(handles.PadRandomZeros, 'Value');
    funParams.WienerFilter = get(handles.WienerFilter, 'Value');
    funParams.HanWindow = get(handles.HanWindow, 'Value');
    funParams.WienerWindowSize =  str2double(get(handles.edit_WienerWindowSize, 'String'));
...
function popupmenu_method_Callback(hObject, eventdata, handles)
    ...
    if strcmpi(props{1}{props{2}},'fastbem')
        ...
        %____________ updated by Waddah Moghram on 2019-12-16
        set(handles.PadRandomZeros,'Enable','off');
        set(handles.WienerFilter,'Enable','off');
        set(handles.WienerWindowLabel,'Enable','off');
        set(handles.edit_WienerWindowSize, 'Enable', 'off')
        set(handles.HanWindow,'Enable','off');
        %_____________
        ...
        set(get(handles.groupCornerOptimal,'Children'),'Enable','on');
        %____________ updated by Waddah Moghram on 2019-12-16
        set(handles.PadRandomZeros,'Enable','on');
        set(handles.WienerFilter,'Enable','on');
        set(handles.WienerWindowLabel,'Enable','on');
        set(handles.edit_WienerWindowSize, 'Enable', 'on')
        set(handles.HanWindow,'Enable','on');
        %_____________
    end
...
% handles    structure with handles and user data (see GUIDATA)
   ...
   % ===============================================================================================
function edit_regParam_Callback(hObject, eventdata, handles)
% hObject    handle to edit_regParam (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_regParam as text
%        str2double(get(hObject,'String')) returns contents of edit_regParam as a double


    % ===============================================================================================
function edit_PoissonRatio_Callback(hObject, eventdata, handles)
% hObject    handle to edit_PoissonRatio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_PoissonRatio as text
%        str2double(get(hObject,'String')) returns contents of edit_PoissonRatio as a double


% --- Executes on button press in PadRandomZeros.
function PadRandomZeros_Callback(hObject, eventdata, handles)
% hObject    handle to PadRandomZeros (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of PadRandomZeros


% --- Executes on button press in HanWindow.
function HanWindow_Callback(hObject, eventdata, handles)
% hObject    handle to HanWindow (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of HanWindow
...
% Hint: get(hObject,'Value') returns toggle state of WienerFilter



function edit_WienerWindowSize_Callback(hObject, eventdata, handles)
% hObject    handle to edit_WienerWindowSize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_WienerWindowSize as text
%        str2double(get(hObject,'String')) returns contents of edit_WienerWindowSize as a double


% --- Executes during object creation, after setting all properties.
function edit_WienerWindowSize_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_WienerWindowSize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
