...
if ~isfield(pivData,'imArray1')   % read files only if not read in the previous pass
    ...
    if sum(auxDiff)>0
        error('(ERROR in pivInterrogate.m): PIVsuite:InconsistImgs','Image 1, Image 2 (and possible mask image) are inconsistent in size.');
    end
...
switch lower(pivPar.iaMethod)
    ...
    case {'deflinear','defspline'}
        ...
        switch lower(pivPar.iaMethod)
            ...
            otherwise
                error('(ERROR in pivInterrogate.m): PIVsuite:iaMethod','Unknown iaMethod.');
        end
        ...
        switch lower(pivPar.iaImageInterpolationMethod)
            ...
            otherwise
                error('(ERROR in pivInterrogate.m): PIVsuite:iaImageInterpolationMethod','Unkonwn iaImageInterpolationMethod');
        end
