function [myMesh]=createMeshAndBasisFromAdhesions(x_vec,y_vec,paxImage,displField,pixelSize,mask)
    % createMeshAndBasisFromAdhesions creates mesh from adhesion information in paxImage. x_vec and y_vec serve as mask for mesh nodes.
    %{
    ...
    disp('(createMeshAndBasisFromAdhesions.m): Creating force mesh using adhesion information in paxillin image ...')
    ...
    alpha = input('(createMeshAndBasisFromAdhesions.m): type desired alpha to threshold the image so that it encompass entire cell membrane: ');

    bwPI = imbinarize(pId,alpha);
    ...
    % Get FA mask
    disp('(createMeshAndBasisFromAdhesions.m): detecting focal adhesions ...')
    ...
    for k=1:nFA
        ...
        % from this, we can try to find if there is any points apart by 5 pixel in the mask
    ...
    disp('(createMeshAndBasisFromAdhesions.m): detecting nascent adhesions ...')
    sigmaPSF_NA = 1.48;
    ...
    for j=intPtsID
        ...
        disp(['(createMeshAndBasisFromAdhesions.m): Creating ' num2str(p) 'th force base ... (' num2str(cputime-old_cputime) ' sec passed)'])
    end
    ...
    for j=intPtsID
        ...
        disp(['(createMeshAndBasisFromAdhesions.m): Creating ' num2str(p) 'th basis function ... (' num2str(cputime-old_cputime) ' sec passed)'])
    end
