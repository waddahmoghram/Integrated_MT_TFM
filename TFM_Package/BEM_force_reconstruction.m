function [fx, fy, x_out, y_out, M, pos_u, u, sol_coef, sol_mats] = ...
    ...
    ip.addParameter('PoissonRatio',0.36,@isscalar);                 % Modified by Waddah Moghram from 0.50 and made it not a required parameter
    ...
    ip.addParameter('strictBEM',false,@islogical); 
    ...
    %% Modified by Waddah Moghram on 2/24/2019 for GPU
    ...
    E = gpuArray(E);
    L = gpuArray(L);
    x_out = gpuArray(x_out);
    y_out =  gpuArray(y_out);
    
%     pos_u = gpuArray(pos_u);
%     
%     ux = gpuArray(ux);
%     uy = gpuArray(uy);
%     
%     ux_vec = gpuArray(ux_vec);
%     uy_vec = gpuArray(uy_vec);
...
%     y_vec = gpuArray(y_vec);
    ...
    disp('(BEM_force_reconstruction.m): 2.) Building up forward map:...');
    inputFwdMap = false;
...
%                 'basisClassTblPath',basisClassTblPath,'wtBar',wtBar,'imgRows',imgRows,'imgCols',imgCols,'thickness',thickness);    
    ...
    elseif isempty(M)
        ...
        disp('(BEM_force_reconstruction.m): Using input Forward Map');
        inputFwdMap = true;
    end
    toc;
    disp('(BEM_force_reconstruction.m): Done: forward map!');
    ...
    tic;
    disp('(BEM_force_reconstruction.m): 3.) Solve for coefficients, this is memory intensive [~5min]:... ')
...
%         [sol_coef,L] = calculateLfromLcurve(M,MpM,u,Lap,solMethodBEM);
    ...
    elseif strcmpi(solMethodBEM,'1NormReg')
        ...
        disp('(BEM_force_reconstruction.m): Performing sparse deconvolution; adoped from Aster et. al.')
...
%             end
        ...
        disp(['(BEM_force_reconstruction.m): tolerance value: ' num2str(tolx)])
        %Check for nan in u
        idxNonan = ~isnan(u);
...
%         sol_mats.nW=normWeights;
    ...
    elseif strcmpi(solMethodBEM,'1NormRegLaplacian')
        % Now, perform the sparse deconvolution.
        disp('(BEM_force_reconstruction.m): Performing sparse deconvolution with laplacian')

        disp('(BEM_force_reconstruction.m): Building laplacian operator')
        Lap = buildLaplacian(forceMesh);
        % plot the solution for the corner
        %Check for nan in u
        idxNonan = ~isnan(u);
...
%         MgridMid = Mgrid(:,colsOut/2*rowsOut+rowsOut/2);
        ...
        sol_mats.tool='fourier';
            ...
            error(['(ERROR in BEM_force_reconstruction.m): I don''t understand the input for the solution method: ',solMethodBEM])
        end
    ...
    toc;
    disp('(BEM_force_reconstruction.m): Done: coefficients!');
    ...
    disp('(BEM_force_reconstruction.m): 4.) Evaluate solution:... ')
    tic;
    ...
    if nargin >= 10 && strcmp(method,'fast') && ~strictBEM
        ...
        for j=1:2*forceMesh.numNodes
            ...
            if mod(j,20)==0
                disp(['(BEM_force_reconstruction.m):  ', num2str(j) , 'th basis function was processed so far ... time passed: ', num2str(cputime-t)])
                t=cputime;
            end
        end
    end
    toc;
    disp('(BEM_force_reconstruction.m): Done: solution!')
%% ==============================================================================
...
function Lap = buildLaplacian(forceMesh)
    ...
    disp('(BEM_force_reconstruction.m): Building Laplacian Map...')
    tic;
...
function [sol_coef,reg_corner] = calculateLcurve(L,M,MpM,u,eyeWeights,LcurveDataPath,LcurveFigPath,LcurveFactor,varargin)
    ...
    try
        if strcmp(lcornerOptimal,'optimal')
            disp('(BEM_force_reconstruction.m/calculateLcurve): Inflection point smaller than L-corner will be chosen')
            [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L,'inflection',2);
            if isempty(reg_corner)
                disp('(BEM_force_reconstruction.m/calculateLcurve): Inflection point larger than L-corner was not identified. L-corner will be chosen')
        ...
        elseif strcmp(lcornerOptimal,'lcorner')
            disp('(BEM_force_reconstruction.m/calculateLcurve): L-corner will be chosen')
            [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L,'inflection',0); %L-corner
        end
        %     ireg_corner=[];
    ...
    if isempty(ireg_corner) || mod(ireg_corner,1)>0                                                     % if ireg_corner is interpolated
        disp(['(BEM_force_reconstruction.m/calculateLcurve): L-corner regularization parmater L = ' num2str(reg_corner) '... final solution calculation ...'])
...
%%
    %old code
    % display('(BEM_force_reconstruction.m/calculateLcurve): Calculating L-curve ...')
    % b  = M'*u;
...
function [sol_coef,reg_corner] = calculateLcurveSparse(L, M, MpM, u, eyeWeights, maxIter, tolx, tolr, LcurveDataPath,  LcurveFigPath, LcurveFactor, varargin)
    % Input check
    ip =inputParser;
    ip.addParameter('(BEM_force_reconstruction.m/calculateLcurveSparse): lcornerOptimal','optimal',@ischar);
    ip.parse(varargin{:});
    lcornerOptimal = ip.Results.lcornerOptimal;
    ...
    for i=1:length(alphas);
        disp(['(BEM_force_reconstruction.m/calculateLcurveSparse): Testing L = ' num2str(alphas(i)) '... '])
    ...
    if strcmp(lcornerOptimal,'optimal')
        disp('(BEM_force_reconstruction.m/calculateLcurveSparse): Inflection point larger than L-corner will be chosen')
        [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L,'inflection',1); %inflection point larger than l-corner
        if isempty(reg_corner)
            disp('(BEM_force_reconstruction.m/calculateLcurveSparse): Inflection point larger than L-corner was not identified. L-corner will be chosen')
            [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L,'inflection',0); %L-corner
        %     ireg_corner=[];
        %     [reg_corner,rhoC,etaC]=l_corner(rho,eta,alphas);
        end    
    ...
    xlabel('(BEM_force_reconstruction.m/calculateLcurveSparse): Residual Norm ||Gm-d||_{2}');
    ylabel('(BEM_force_reconstruction.m/calculateLcurveSparse): Solution Norm ||m||_{1}');
    hold on
    % mark and label the corner
    ...
    disp('(BEM_force_reconstruction.m/calculateLcurveSparse): Displaying the 1-norm L-curve')
    % print -deps2 nameSave
    % print(hLcurve,strcat(nameSave,'.eps'),'-depsc')
    disp(['(BEM_force_reconstruction.m/calculateLcurveSparse): Saving ' , LcurveFigPath, '.fig in progress'])
    saveas(hLcurve,LcurveFigPath);
    
    disp(['(BEM_force_reconstruction.m/calculateLcurveSparse): Saving ' , LcurveDataPath, '.mat in progress'])
    save(LcurveDataPath,'rho','eta','reg_corner','ireg_corner','alphas','rho_corner','eta_corner','msparse','-v7.3');
    close(hLcurve)
    
     disp('---------------------------------------------')
    if mod(ireg_corner,1)>0 % if ireg_corner is interpolated
        disp(['(BEM_force_reconstruction.m/calculateLcurveSparse): L-corner regularization parmater L = ' num2str(reg_corner) '... final solution calculation ...'])
