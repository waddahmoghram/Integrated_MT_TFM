...
if nargin<2 || nargin>4
    error('(ERROR in padarrayXT.m): Incompatible number of input arguments.');
end
...
if nargin > 2
    ...
    for k = firstStringToProcess:nargin
        ...
        switch string
            ...
            otherwise
                error(message('(ERROR in padarrayXT.m): images:padarray:unexpectedError'))
        end
    end
end
...
if strcmp(method,'constant') && ~(isnumeric(a) || islogical(a))
    error(message('(ERROR in padarrayXT.m): images:padarray:badTypeForConstantPadding'))
end
