function [pos_f, force, sol_coef]=calcSolFromSolMatsFastBEM(M, sol_mats, x, y, ux, uy, forceMesh, L, x_out, y_out, varargin)
    ...
    %if no points of interests are specified, i.e x_out, y_out, then forces are calculated on the nodes of the force mesh:   
    ...
    % See BEM_force_reconstruction for a nice explanation of the next steps:
    ...
    %     eyeWeights = sol_mats.eyeWeights;
        ...
        if sol_L==L %&& sum(sol_nW~=normWeights)==0
            ...
            error('(ERROR in calcSolFromSolMatsFastBEM.m): Weights or regularization parameter have been changed. QR cannot be reused!')
        end
    ...
    %     eyeWeights =diag(normWeights);
        ...
        error('(ERROR in calcSolFromSolMatsFastBEM.m): No solution matrices have been found')
    end
...
function [sol_coef,reg_corner] = calculateLfromLcurveSparse(L,M,MpM,u,eyeWeights,maxIter,tolx,tolr,LcurveDataPath,LcurveFigPath,LcurveFactor)
    ...
    for i=1:length(alphas);
        disp(['(calcSolFromSolMatsFastBEM.m\calculateLfromLcurveSparse): Testing L = ' num2str(alphas(i)) '... '])
    ...
    disp('(calcSolFromSolMatsFastBEM.m\calculateLfromLcurveSparse): Displaying the 1-norm L-curve')
    ...
    if mod(ireg_corner,1)>0 % if ireg_corner is interpolated
        disp(['(calcSolFromSolMatsFastBEM.m\calculateLfromLcurveSparse): L-corner regularization parmater L = ' num2str(reg_corner) '... final solution calculation ...'])
