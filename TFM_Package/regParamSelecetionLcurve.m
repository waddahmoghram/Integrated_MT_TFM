...
elseif isempty(maxKappaCandIdx)
    disp('(regParamSelecetionLcurve.m): There is no local maximum in curvature in the input lambda range.Using global maximum instead ...');
...
if inflection == 1 % if inflection point larger than lcorner is to be chosen.
    ...
    disp(['(regParamSelecetionLcurve.m): L-inflection value (larger than L-corner): ' num2str(reg_corner)])
elseif inflection == 2 % if inflection point smaller than lcorner is to be chosen.
    ...
    ireg_corner= inflectionIdx+3;%round((maxKappaIdx+maxKappaDiffIdx)/2); % thus we choose the mean of those two points.
    reg_corner = lambda_cut(inflectionIdx);
    disp(['(regParamSelecetionLcurve.m): L-inflection value (smaller than L-corner): ' num2str(reg_corner)])
else
    ...
    disp(['(regParamSelecetionLcurve.m): L-corner regularization parameter value: ' num2str(reg_corner)])
end
...
% 
% disp(['(regParamSelecetionLcurve.m): Initial L-corner regularization parameter value: ' num2str(reg_corner) '.'])
% 
% % poly-fit version
...
%     disp(['(regParamSelecetionLcurve.m): Sub-knot resolution L-corner regularization parameter value: ' num2str(reg_corner) '.'])
% else
%     disp('(regParamSelecetionLcurve.m): The corner''''s L-corner does not have positive curvature')
...
%     x_cut = x((numCutPoints+1:end));    
    h=figure('color', 'w'); set(h,'Position',[1000,100,350,600])
    subplot(3,1,1),plot(x,y,'k')
    title(sprintf('regularization parameter = %0.5g', ireg_corner));
...
% % part below commented by WIM on 2/18/2019. Uncommented on 2020-05-09
%     if ireg_corner-3>0
