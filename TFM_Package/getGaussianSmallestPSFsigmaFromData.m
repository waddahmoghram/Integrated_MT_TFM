%{
    ...
    fixed by Waddah Moghram on 2019-11-10

%}
function [sigma, sigmaPrimary, plotHandle] = getGaussianSmallestPSFsigmaFromData(imageList, varargin)
...
try
    rng('default'); % Reset seed for common start values
    ...
    if ip.Results.Display
        ...
        %         h = bar(si,ni,'histc');        
        h = bar(si(1:end-1), ni, 'histc');              % fixed by Waddah Moghram on 2019-11-10
        set(h, 'FaceColor', 0.6*[1 1 1], 'EdgeColor', 'none');
        plotHandle = gcf;
        hold on;
        plot(si, pdf(obj,si'), 'r');
        for i = setdiff(1:3, idx)
            plot(si, amp(i)*normpdf(si, mu(i), svec(i)), 'k', 'HandleVisibility', 'on')
        end
