function [reg_grid, xvec, yvec, gridSpacing] = createRegGridFromDisplFieldOdd(displField, mag, edgeErode)
    %{
        ...
        Updated by Waddah Moghram on 2019-02-22 to take advantage of the computer GPUs
        Updated by Waddah Moghram on 2019-11-11 to create grid with odd number of points.
    %} 
...
%% Parse input variables
    ...
    %% Added by Waddah Moghram on 2/22/2019
    if useGPU
        if isa(displField, 'struct')
            displFieldPos = gpuArray(displField(1).pos);
        else
            displFieldPos = gpuArray(displField);
        end
    else
        if isa(displField, 'struct')
            displFieldPos = displField(1).pos;
        else
            displFieldPos = displField;
        end
    end
    
    %%    First get the corners (here we take field no.1 but that is arbitrary):
    LeftUpperCorner(1:2) = floor([min(displFieldPos(:,1)), min(displFieldPos(:,2))]);
    RightLowerCorner(1:2) = ceil([max(displFieldPos(:,1)), max(displFieldPos(:,2))]);
    ...
    % Modified by WIM on 2/22/2019    
    if useGPU
        numBeads = gpuArray(length(displFieldPos));
    else
    ...
    %disp('Interbead Distance has been upscaled in createRegGridFromDisplField. This has to be taken out again:')
    ...
    % Updated by Waddah Moghram on 2019-11-11 to get an even grid for FTTC caluclations to reduce noise
    % Make sure that we get an even Number of pieces which leads to an even  number of grid points later:
    numPieces = numPieces + mod(numPieces,2);

    % The size of the new grid is then given by:
    newSize = numPieces * gridSpacing;

    % If there is not much difference to the old size, we need to further erode the edges (this part is optional and could be skipped):
    % edgeErode=1;
    if edgeErode==1
    ...
    xVecGrid =newLeftUpperCorner(1):gridSpacing:newRightLowerCorner(1);
    yVecGrid =newLeftUpperCorner(2):gridSpacing:newRightLowerCorner(2);

    % Now we have everything needed for making the new grid:
    [X, Y] = meshgrid(xVecGrid, yVecGrid);
    reg_grid(:,:,1) = X';
    reg_grid(:,:,2) = Y';
    ...
    if useGPU
        [reg_grid, xvec, yvec, gridSpacing] = gather(reg_grid, xvec, yvec, gridSpacing);
    end