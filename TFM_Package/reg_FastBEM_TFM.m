function [pos_f, force, forceMesh, M, pos_u, u, sol_coef, sol_mats] = ...
    ...
    % Commented by WIM on 2/4/2019. 
%     ip.addRequired('pRatio',@(x) isequal(pRatio,.5));
    ip.addRequired('pRatio',@isscalar);
   ...
   if ~isa(grid_mat, 'gpuArray')
        grid_mat = gpuArray(grid_mat);
   end
    
    %%
    if isempty(grid_mat)
        % If no mesh is specified for the forces, we create a hexagonal mesh that will be centered in the field of view. 
        %Here, only the first frame will be used:
    ...
    disp('===================================')
    disp('(reg_FastBEM_TFM.m): 1.) Creating mesh & basis [~5sec]:...');
    tic;
    
    % keepBDPts=true;        %this might lead to unmatching forward map that lead to diagonalized traction map
    ...
    toc;
    disp('(reg_FastBEM_TFM.m): Done: mesh & basis!');
