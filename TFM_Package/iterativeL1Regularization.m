...
%{
    mreg = iterativeL1Regularization(G, GTG, d, L, alpha, maxiter, tolx, tolr, doPlot, forceMesh)
    solves L1 regularization problem with:
        forward matrix G, 
        displacement vector d, 
        regularization parameter alpha, 
        semi-norm matrix L.
        tolx 
    & m_diff are used for convergence criteria
...
%} 
    ...
    if nGPU>0
        disp(' (iterativeL1Regularization.m): GPU used')
        gD = gpuDevice();
        memGTG = whos('GTG');
        memL = whos('L');
    ...
    %     Lgpu = gpuArray(L);
            ...
            iter=1;
            disp(['(iterativeL1Regularization.m): L1 Norm regularization (iteration:' num2str(iter) ')....'])
            ...
            while (iter < maxiter)
                ...
                disp(['((iterativeL1Regularization.m): iteration:' num2str(iter) ')'])
                ...
                if (norm(m-mold)/(1+norm(mold)) < tolx) %|| norm(m-mold)<m_diff
                    mreg=m;
                    display(['(iterativeL1Regularization.m): norm(m-mold)=' num2str(norm(m-mold)) ', 1+norm(mold)=' num2str(1+norm(mold)) ', norm(m-mold)/(1+norm(mold))=' ...
    ...
    if ~usedGPU
        disp('(iterativeL1Regularization.m): No GPU used')
        ...
        while (iter < maxiter)
          ...
          if doPlot
              ...
              colorbar
              colormap jet
              title(['(iterativeL1Regularization.m): iteration number=' num2str(iter-1)])
        ...
        %   display('(iterativeL1Regularization.m): LSQR...')
        %   tic
        ...
        %   display('(iterativeL1Regularization.m): QR...')
        %   tic
        ...
        disp('(iterativeL1Regularization.m): L1 norm regularization: maximum iterations exceeded.')
        display(['(iterativeL1Regularization.m): Final ratio= norm(m-mold)/(1+norm(mold))='  num2str(norm(m-mold)/(1+norm(mold)))]);
        mreg=m;
    end