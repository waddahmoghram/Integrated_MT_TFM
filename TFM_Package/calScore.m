function [score,blockIsTooSmall,vP,vF] = calScore(kym,centerI,corL,vP,vF,varargin)
    ...
    % 
    % 
    % For boundary points, the cutoff of background can make the effective image area too small for stable correlation
    % We check this and report it back as output.
    %% ======================================================================================================
    blockIsTooSmall = 0;
    ...
    elseif bCont && (numFrames==2)
        %------------  normalized cross-correlation with continuous window shift (by interpolation) 
    ...
    elseif min(min(kymMask(:,:,1))) == 1 || min(min(kymMask(:,:,1))) == 0
        %------------ %Background intensities are set to be zero. So, if there is no zero intensities, there is no background.
        kymP2 = kym.*kym;

        %------------ The norm of the kymographed image band at each frame.
        bNorm1 = sqrt(sum(sum(sum(kymP2(bI1,bI2,1:numFrames-1)))));

        %------------  fft-based cross-correlation. This can reduce computation time especially for large velocity range - Sangyoon
        K1 = length(bI1); K2 = length(bI2);
        N1 = length(bI1e); N2 = length(bI2e);
...
%         toc;
        ...
        if numFrames == 2
            %------------  For image stack of 2, fft-based crosscorrelation is faster and make 
            % the correlation less sensitive to bright region in template window - Sangyoon
            score_nxc2 = normxcorr2(kym(bI1,bI2,1:numFrames-1), kym(bI1e,bI2e,2:numFrames));
            score = score_nxc2(K1:N1,K2:N2);                    % normalized
        else
        ...
        % area) is bigger than 'bAreaThreshold'. Note: Background pixel values are zero.
        % 
