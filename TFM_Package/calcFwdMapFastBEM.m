function [M]=calcFwdMapFastBEM(x_vec_u, y_vec_u, forceMesh, E,varargin)
    ...
    %% Added by Waddah Moghram on 2/24/2019
    x_vec_u = gpuArray(x_vec_u);
    y_vec_u = gpuArray(y_vec_u);
...
%         method='*cubic';
    ...
    %% Modified by Waddah Moghram on 2/24/2019
%     ux=zeros(length(x_vec_u),2*numBasis);
%     uy=zeros(length(y_vec_u),2*numBasis);
    ux=zeros(length(x_vec_u),2*numBasis,'gpuArray');
    uy=zeros(length(y_vec_u),2*numBasis,'gpuArray');
