...
%} 
    ...
    LeftUpperCorner(1:2) = [min(displFieldPos(:,1)), min(displFieldPos(:,2))];
    RightLowerCorner(1:2) = [max(displFieldPos(:,1)), max(displFieldPos(:,2))];
    ...
    % from that calculate the edge length within the hexagonal grid (the edge length of the regular triangles.):
    if forceInteger==1
    ...
    % then the height of the triangles is:
    if forceInteger ==1
    ...
    % The hexagonal grid is the superposition of two regular grids with
        % yspacing=edgeLength and
        % xspacing=2*triHight with a relative
        % yshift=edgeLength/2 and 
        % xshift=triHight.
    ...
    if sizeDiff(1)<0 || sizeDiff(2)<0
        disp('(ERROR in createHexGridFromDisplField.m): something went wrong! Nothing has been done');
        return;
    end
