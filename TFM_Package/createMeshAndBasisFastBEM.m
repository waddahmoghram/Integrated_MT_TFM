...
% to make it very efficient, x_vec and y_vec should be such that they yield a hexagonal lattice.
%{
...
%}
    ...
    x_vecCPU = gather(x_vec);
    y_vecCPU = gather(y_vec);
    
    if ~isSq
        %delaunay_mesh=delaunay(x_vec,y_vec);        
        %% Fixed by WIM:P dt.Points and dt.ConnectivityList are the newer variables.
        dt= delaunayTriangulation(x_vecCPU,y_vecCPU);
        p = dt.Points;
...
%         % we read this out because the access of the Delaunay-structure takes so long:
%         % bdPtsID = convexHull(dt);
        p = dt.X;
        t = dt.Triangulation; 
    end
    
    % find the neighbors to each node, the bounds of each basis function and the convex hull of the grid:
    ...
    for j=1:myMesh.numNodes
        % classify the basis function. check if the current basis function falls into an existing basis class:
        noClass=false;
        foundClass=[];
        ...
        if isempty(basisClass)
            ...
            elseif length(foundClass)>1
                error('(ERROR in createMeshAndBasisFastBEM.m): Basis Classes should be unique, something went wrong!')
            end
        end
        ...
        if ~keepBDPts
            % test if the current pt is part of the boundary. If yes, the criteria that the pts is not on the bd is not fulfilled.
            bdCriteriaOK = ~ismember(j, bdPtsID);         
        end
        if noClass && bdCriteriaOK
            % sort in the position of the neighboring nodes. The center node is positioned at the origin.
            ...
            %% Modified by WIM on 2/24/2019. Consider using plain-old delauney() since it is GPU compatible.
%             dtBaseSup= delaunayTriangulation(gather(allPts(:,1)),allPts(:,2));
            dtBaseSup= delaunayTriangulation(gather(allPts(:,1)),gather(allPts(:,2)));
            %%
            ...
            % There might be nans if the points where the function is evaluated is not within the support.     
            ...
            %% Modified by WIM on 2/24/2019. Consider using convexHull()
%             [~, areaSupport] = convhull(allPts(:,1),allPts(:,2));
            [~, areaSupport] = convhull(gather(allPts(:,1)),gather(allPts(:,2)));            
            %%
            ...
            %% Modified by WIM on 2/24/2019
%             [~, unitVolume] = convhulln(X);
            [~, unitVolume] = convhulln(gather(X));            
            %%
