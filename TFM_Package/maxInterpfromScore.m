...
if (maxI2(1)-subv)>=1 && (maxI2(1)+subv)<=size(score,1)...
   && (maxI2(2)-subv)>=1 && (maxI2(2)+subv)<=size(score,2) %|| strcmp(mode,'CDWS')
    ...
    % ---------------- added by Waddah Moghram on 2/21/2019
    
    if isa(bsp,'gpuArray')
        s = fitoptions('Method','NonlinearLeastSquares','StartPoint', [gather(asp),gather(bsp),gather(csp),gather(dsp),gather(esp)]);     % Added Gather on 2/21/2019.
    else
        s = fitoptions('Method','NonlinearLeastSquares','StartPoint', [asp,bsp,csp,dsp,esp]);     % Added Gather on 2/21/2019.
    end
    f = fittype('a*x^2+b*x+c*y^2+d*y+e','independent', {'x', 'y'}, 'dependent', 'z','option',s);
    
    if isa(subvF1D,'gpuArray')
        if isa(sub_score1D,'gpuArray')
            sf = fit( [gather(subvF1D), gather(subvP1D)], gather(sub_score1D), f);
        else
            sf = fit( [gather(subvF1D), gather(subvP1D)], sub_score1D, f);
        end
    else
        sf = fit( [subvF1D, subvP1D], gather(sub_score1D), f);
    end
...
if (~bPolyTracked || norm(maxVorg-maxV2,2)>1) %&& ~strcmp(mode,'CDWS') %checking for proximity of the fitted point to the original discrete point strcmp(mode, 'accurate') || 
    ...
    sub_score = score(max(1,maxI2(1)-subv):min(size(score,1),maxI2(1)+subv),...
                        max(1,maxI2(2)-subv):min(size(score,2),maxI2(2)+subv));
    % my field of interest    % Modified by WIM on 2/21/2019
    subvP = gather(vP(max(1,maxI2(1)-subv):min(size(score,1),maxI2(1)+subv)));
    subvF = gather(vF(max(1,maxI2(2)-subv):min(size(score,2),maxI2(2)+subv)));
    
    sp   = csape({subvP,subvF},gather(sub_score));
    dsp1 = fnder(sp,[1,0]);
    dsp2 = fnder(sp,[0,1]);
...
%             options = optimset('Algorithm','sqp');% this too
    ...
    maxVorg = gather(maxVorg);    % Added by WIM on 2/21/2019
