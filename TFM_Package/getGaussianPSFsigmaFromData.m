...
% Updated by Waddah Moghram on 2019-10-29

function [sigma, sigmaPrimary] = getGaussianPSFsigmaFromData(imageList, varargin)
...
try
    ...
    if ip.Results.Display
        ...
        ylabel('Frequency');
        sigmaPrimary = mu(idx); 
        hl = legend(' Measured distribution', ' Mixture model', 'Primary mode', ['\sigma = ' num2str(sigmaPrimary, '%.2f')]); 
        set(hl, 'Box', 'off');
        formatTickLabels();
        drawnow;
    end
