function [ux, uy, x_grid, y_grid, meshPtsFwdSol] = ...
        fwdSolution(x0, y0, E, xmin, xmax, ymin, ymax, force_x, force_y, method, opt, meshPtsFwdSol, h, v, refine, useSameSampling)
    %{
    This forward solution is only valid for a Poisson's ratio v = 0.5 if not specified.
    ...
    Waddah Moghram 2019
        v.2020-02-21 by WIM reverted back to cubic interpolation with griddata. Use -v4 for NaNs instead of 0s.
    %}
...
%________________ ADDED BY WIM On 2020-02-21 isntead of the part above
            ux(i) = integral2(integrandx,xmin,xmax,ymin,ymax,'AbsTol',5e-6, 'Method', 'iterated');% RelTol sucks! 'RelTol',5e-13);
            uy(i) = integral2(integrandy,xmin,xmax,ymin,ymax,'AbsTol',5e-6, 'Method', 'iterated');% RelTol sucks! 'RelTol',5e-13);
...
%         disp('------------------Use fast-fourier transform convolution--------------')    
        %{
        ***************************************************************
        ...
        ***************************************************************
        ...
        % To account for dx*dy in the convolution integral one has to finally rescale the result by the following scaling factor:
    ...
    %     [xgrid_G,ygrid_G] = meshgrid(yvec_G,xvec_G);
        ...
        % Make force_x and force_y a function handle if it is a matrix
            ...
            %     yvec=ymat(:);
                ...
                force_x_vec=force_x(:);
                force_y_vec=force_y(:);
%                 %%_________________________ Modified by WIM
%                 force_x = scatteredInterpolant(gather(xvec),gather(yvec),gather(force_x_vec));
%                 force_y = scatteredInterpolant(gather(xvec),gather(yvec),gather(force_y_vec));
%             end
%             %% added by WIM
%             xgrid_F = gather(xgrid_F);
%             ygrid_F = gather(ygrid_F);
%             %%
...
%             discrete_Force_y_unPadded = force_y(xgrid_F,ygrid_F);
            %__________________ Modified on 2020-02-21 by WIM to revert back to 'cubic' fit followed by '-v4' to keep things consistent!
            end
            discrete_Force_x_unPadded = griddata(gather(xvec), gather(yvec), gather(force_x_vec), gather(xgrid_F), gather(ygrid_F), 'cubic');
            discrete_Force_y_unPadded = griddata(gather(xvec), gather(yvec), gather(force_y_vec), gather(xgrid_F), gather(ygrid_F), 'cubic');
 
            if useGPU
                try
                    discrete_Force_x_unPadded = gpuArray(discrete_Force_x_unPadded);
                    discrete_Force_y_unPadded = gpuArray(discrete_Force_y_unPadded);
                catch
                    useGPU = false;
                end                   
            end
            %____________________
            %______________ WIM 2020-02-21 Replaced part that replaced NaNs with zeros by interpolating using -V4 griddata() from the surroundings instead.
%             % taking care of nans
...
%             discrete_Force_y_unPadded(checkVec) = 0;
            MaskSizePerSide = min(15, round(sqrt(size(xvec,1))));                        % choose 4 grid points on either side.
            %__________________________ CORRECTING for NaNs out of griddata(cubic) using griddata(v4)
            NANindX = find(isnan(discrete_Force_x_unPadded(:)));
            NANindY = find(isnan(discrete_Force_y_unPadded(:)));
            NANind = unique(horzcat(NANindX, NANindY));          % combine poins together            

            if ~isempty(NANindX) || ~isempty(NANindY)
%                 warning('NaN values where found')           
                [NANindX, NANindY] = ind2sub(size(discrete_Force_x_unPadded), NANind);
                NANindXRange = [NANindX - MaskSizePerSide, NANindX + MaskSizePerSide];
                NANindYRange = [NANindY - MaskSizePerSide, NANindY + MaskSizePerSide];
                % If negative range or more than max, choose the first grid point. 
                NANindXRange(logical(NANindXRange(:,1) <= 1), 1) = 1;
                NANindYRange(logical(NANindYRange(:,1) <= 1), 1) = 1;  
                NANindXRange(logical(NANindXRange(:,2) >= size(discrete_Force_x_unPadded,1)), 2) = size(discrete_Force_x_unPadded,1);
                NANindYRange(logical(NANindYRange(:,2) >= size(discrete_Force_x_unPadded,2)), 2) = size(discrete_Force_x_unPadded,2);
                for ii = 1:numel(NANind)
                    % NOTE: using the line below crashes for very large grid sizes as input;
                    XI_ii = xgrid_F(NANindXRange(ii,1):NANindXRange(ii,2), NANindYRange(ii,1):NANindYRange(ii,2));
                    YI_ii = ygrid_F(NANindXRange(ii,1):NANindXRange(ii,2), NANindYRange(ii,1):NANindYRange(ii,2));
                    discrete_Force_x_ii = discrete_Force_x_unPadded(NANindXRange(ii,1):NANindXRange(ii,2), NANindYRange(ii,1):NANindYRange(ii,2));
                    discrete_Force_y_ii = discrete_Force_y_unPadded(NANindXRange(ii,1):NANindXRange(ii,2), NANindYRange(ii,1):NANindYRange(ii,2));

                    xgrid_F_ii_NoNaN = XI_ii(~logical(isnan(discrete_Force_x_ii)));
                    ygrid_F_ii_NoNaN = YI_ii(~logical(isnan(discrete_Force_y_ii)));                 
                    discrete_Force_x_ii_NoNaN = discrete_Force_x_ii(~logical(isnan(discrete_Force_x_ii)));
                    discrete_Force_y_ii_NoNaN = discrete_Force_y_ii(~logical(isnan(discrete_Force_y_ii)));

                    discrete_Force_x_unPadded(NANind(ii)) = griddata(gather(xgrid_F_ii_NoNaN), gather(ygrid_F_ii_NoNaN), gather(discrete_Force_x_ii_NoNaN), gather(xgrid_F(NANind(ii))), gather(ygrid_F(NANind(ii))), 'v4'); 
                    discrete_Force_y_unPadded(NANind(ii)) = griddata(gather(xgrid_F_ii_NoNaN), gather(ygrid_F_ii_NoNaN), gather(discrete_Force_y_ii_NoNaN), gather(xgrid_F(NANind(ii))), gather(ygrid_F(NANind(ii))), 'v4');  
                end            
            end
...
%         ux_grid=real(ux_grid);
%         uy_grid=real(uy_grid);
%%
...
%         v=0.5;
    ...
    %     refine =true;
        if ~isempty(xmin) && ~isempty(xmax) && ~isempty(ymin) && ~isempty(ymax) && refine
            ...
            iux_fine = griddata(x_grid_fine,y_grid_fine,ux_fine,xgrid_F,ygrid_F,'cubic');%'*cubic'
            iuy_fine = griddata(x_grid_fine,y_grid_fine,uy_fine,xgrid_F,ygrid_F,'cubic');%'*linear'
            ...
            if nargin>10 && strcmp(opt,'noIntp')
                ...
                %% Modified by WIM on 2/22/2019.Removed '*cubic' and made it just 'cubic'
