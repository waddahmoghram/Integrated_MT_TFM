...
StrainEnergyProcessParameters = parseProcessParams(strainEnergyCalculationProcess,paramsIn);

%% Output setup
%% Backup the original vectors to backup folder
if exist(StrainEnergyProcessParameters.OutputDirectory,'dir')
    contents=dir(StrainEnergyProcessParameters.OutputDirectory);
    if any(arrayfun(@(x) ~x.isdir,contents)) % There should be something inside. Otherwise, backing up
        ...
        ii = 1;
        backupFolder = [StrainEnergyProcessParameters.OutputDirectory ' Backup ' num2str(ii)];
        while exist(backupFolder,'dir')
            backupFolder = [StrainEnergyProcessParameters.OutputDirectory ' Backup ' num2str(ii)];
            ii=ii+1;
        end
        mkdir(backupFolder);
        copyfile(StrainEnergyProcessParameters.OutputDirectory, backupFolder,'f')
    end
end
mkClrDir(StrainEnergyProcessParameters.OutputDirectory);
%% --------------- Initialization ---------------%%
if feature('ShowFigureWindows')
    wtBar = waitbar(0,'Initializing...','Name',strainEnergyCalculationProcess.getName());
end
...
% tractionImgFolder=StrainEnergyProcessParameters.OutputDirectory;
...
outputFile{1,1} = [StrainEnergyProcessParameters.OutputDirectory filesep 'strainEnergyInFOV.mat'];
outputFile{1,2} = [StrainEnergyProcessParameters.OutputDirectory filesep 'strainEnergyInCell.mat'];
outputFile{1,3} = [StrainEnergyProcessParameters.OutputDirectory filesep 'forceBlobs.mat'];
if StrainEnergyProcessParameters.exportCSV
    outputFile{1,4} = [StrainEnergyProcessParameters.OutputDirectory filesep 'strainEnergyInFOV.csv'];
    outputFile{1,5} = [StrainEnergyProcessParameters.OutputDirectory filesep 'totalForceInFOV.csv'];
    outputFile{1,6} = [StrainEnergyProcessParameters.OutputDirectory filesep 'strainEnergyInCell.csv'];
    outputFile{1,7} = [StrainEnergyProcessParameters.OutputDirectory filesep 'totalForceInCell.csv'];
    outputFile{1,8} = [StrainEnergyProcessParameters.OutputDirectory filesep 'strainEnergyInForceBlobs.csv'];
    outputFile{1,9} = [StrainEnergyProcessParameters.OutputDirectory filesep 'indivForceInForceBlobs.csv'];
end    
outputFile{1,10} = [StrainEnergyProcessParameters.OutputDirectory filesep 'MovieStrainEnergyData_all.mat'];

%---------- Added by Waddah Moghram on 8/21/2019
outputFile{1,11} = [StrainEnergyProcessParameters.OutputDirectory filesep 'StrainEnergyParameters.mat'];
outputFile{1,12} = [StrainEnergyProcessParameters.OutputDirectory filesep 'movieDataBefore.mat'];
outputFile{1,13} = [StrainEnergyProcessParameters.OutputDirectory filesep 'movieDataAfter.mat'];    
    
%---------------------------


strainEnergyCalculationProcess.setOutFilePaths(outputFile);
...
save(outputFile{1,11},  'strainEnergyCalculationProcess', 'StrainEnergyProcessParameters', 'movieDataBefore','forceFieldProc',...
    'CorrectedDisplFieldProc', 'iMaskProcess','iSDCProc','minSize', 'minTraction', 'nTopBlobs','-v7.3');
MD = movieData;
...
for ii=1:nFrames
    ...
    if StrainEnergyProcessParameters.useFOV || 1 % I will calculate this anyway
        % segment
    ...
    if existMask && StrainEnergyProcessParameters.useCellMask
    ...
    if StrainEnergyProcessParameters.performForceBlobAnalysis
...
logMsg='Saving...';
if feature('ShowFigureWindows'), waitbar(0,wtBar,sprintf(logMsg)); end
if StrainEnergyProcessParameters.useFOV || 1
...
if StrainEnergyProcessParameters.useCellMask
