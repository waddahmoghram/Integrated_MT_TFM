...
% % SDC proc
% % Lines below commented out and corrected by WIM on 2/18/2019
% try
...
% Corrected by WIM. Should be reference frame, not dispMaps.mat on 2/18/2019
...
% Draw the ROI
disp(['(setROIfromForcemap.m): Draw rectangle for ROI for ' MD.movieDataPath_ '.'])
if isempty(MD.roiMaskPath_)
    h=imrect;
else
    roiMask=imread(MD.roiMaskPath_);
    boundROI=bwboundaries(roiMask);
    % if-statement added by WIM on 2/18/2019
    if ~isempty(boundROI)
...
close(h2)
disp('(setROIfromForcemap.m): ROI created!')
