...
%{
    ...
    Waddah Moghram, Last modified 2019-06-08
%}

% ------------ Added by Waddah Moghram
...
%% Convert Key Variables to GPU Arrays. Added by Waddah Moghram on 2019-02-21. updated on 2019-06-08
    if useGPU
        stack = gpuArray(stack);
        points = gpuArray(points);
        minCorL = gpuArray(minCorL);
        maxCorL = gpuArray(maxCorL);
        maxSpd = gpuArray(maxSpd);
        minFeatureSize = gpuArray(minFeatureSize);
        bgAvgImg = gpuArray(bgAvgImg);
    end
...
%% % SH: Poly-fit version
    ...
    bAreaThreshold = min(0.99*minCorL^2,maxCorL^2*0.1);
    ...
    %-----------------------------------------
    %Calculate the correlation coefficient for each sampling velocity at each point.
    startTime = cputime;
    ...
    if isempty(poolobj)
        poolsize = str2num(getenv('NUMBER_OF_PROCESSORS')) - 1;          % Modified by Waddah Moghram on 12/10/2018 and is better to get all cores.
...
%      for k = 1:nPoints
    ...
    %     maxPerpSpd = initMaxPerpSpd;
        ...
        while pass == 0 && corL <= maxCorL
            ...
            %Create kymograph around each point. 'bandDir' is used as the direction of the kymographed line.
            xI = round(x(k));
            yI = round(y(k));
    ...
    %             
                ...
                for k2 = 1:numFrames
                    %kym(:,:,k2) = double(stack(yI-hCWL:yI+hCWR,xI-hCLL:xI+hCLR,k2));           % Converted back to regular array. GPU array not compatible with cells.
                    kym(:,:,k2) = gather(double(stack(yI-hCWL:yI+hCWR,xI-hCLL:xI+hCLR,k2)));    % Added by WIM on 2/21/2019
                end
    ...
    %                                 end
        ...
        if pass && strcmp(mode,'accurate')
            ...
            halfCorL=(corL-1)/2;
            refineFactor = 10; %round(10*20/corL);   % 10;      %round(10*20/corL); % by this, the pixel value will be magnified.
    ...
    %-----------------------------------------Added by WIM ON 2019-02-21. updated on 2019-06-21
    if useGPU
        v = gather(v);
        corLength = gather(corLength);
    end
    %-----------------------------------------
