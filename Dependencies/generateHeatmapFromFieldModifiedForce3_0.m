function [umMap,XI,YI] = generateHeatmapFromFieldModifiedForce3_0(Field,dataPath,band,ummax,cmapmode,w,h,plotQuiver)
% Written by Waddah Moghram on 2/15/2019
...
%     end
    ...
    VisiblePlot = 'on';       % added for now
    %     plotQuiver = false                        % added by Waddah Moghram, since I could not get to insert only the last argument as false.
    First = 1;    % Tweak it if thsi process gets stopped got stopped
%     numNonEmpty = 800;
    
    % Consider using switch nargin and write down what the default values should be for each case.
    % Also consider input parser to specify particular variables
    ...
    h3 =  figure('color','w','visible',VisiblePlot);    %added by WIM on 2/7/2019. To show, remove 'visible'
    ...
    for FrameNum=First:numNonEmpty
        ...
        if nargin >=2
            ...
            image(umMap)                                  % replaced imshow() with image and colorbar. That should be enough
...
%             subplot('Position',[0.90 0.05 0.25 0.9])
            ...
            set(hc,'Fontsize',18,'FontWeight','bold')
...
% 
%     
%     FirstFrame = 1;     % Tweak it as you wish 
%     for FrameNum=FirstFrame:numNonEmpty
%         msg = ['Frame #',num2str(FrameNum)];
%         disp(msg)
% 
%         [grid_mat,iu_mat,~,~] = interp_vec2grid(Field(FrameNum).pos, Field(FrameNum).vec,[],reg_grid);
...
%             end
% 
%             subplot('Position',[0.9 0.06 0.25 0.88])
%             axis tight
%             caxis([ummin ummax]), axis off
%             hc = colorbar('West');
%             set(hc,'Fontsize',16)
